...
/**
 * Created by thc on 2019/10/11 9:17
 */
@RestController
...
//                        lectureDTOPage.getContent().remove(L);
                    }
                }else {
...
//                        lectureDTOPage.getContent().remove(L);
        ...
        for (LectureDTO L : lectureDTOPage.getContent()){
            ...
            if (L.getIsMatch() != 0){
                lectureDTOList.add(L);
            }
        }
//        response.addHeader("Access-Control-Allow-Origin", "*");   使用nginx需要加
        return ResultVOUtil.success(lectureDTOList,count);
    }
     ...
     */
    @GetMapping("/listc")
    public ResultVO<List<Lecture>> listc(@RequestParam(value = "page",defaultValue = "1") Integer page,
                                        @RequestParam(value = "limit",defaultValue = "10") Integer limit,
                                        LectureDTO lectureDTO,
                                        HttpSession session) throws UnsupportedEncodingException {
